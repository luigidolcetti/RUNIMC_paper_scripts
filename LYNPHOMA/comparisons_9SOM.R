geo_list_BO<-sf::st_read(
  "C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/analysis/BODEN/ex.sqlite"
)

geo_list_RU<-sf::st_read(
  "C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/analysis/RUNIMC1/topLayers.sqlite"
)
...
saveRDS(fsom,
        file = "C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001//som_3x3"  )
...
pdf(file="C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/heatmap_3x3.pdf",
    width = 5,
    height = 5
)
...
for (i in 1:3){
  postscript(file = paste0("C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/3x3_BO_map_",i,".eps"),
             height = 10,
             width = 50,
             horizontal = F,
             bg = 'white',
  ...
  plot(geo_list_BO_annot[geo_list_BO_annot$uid==uids[i],]['SOM'],
       ...
       key.length = 1)
  dev.off()
  postscript(file = paste0("C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/3x3_RU_map_",i,".eps"),
             height = 10,
             width = 50,
             horizontal = F,
             bg = 'white',
...
pdf(file="C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/3x3_heatmap_prePost.pdf",
    width = 10,
    height = 3.5
)
...
tableSOM<-pbapply::pblapply(1:3,function(i){
  ...
  out<-cbind(BO/sum(BO),BO_area/sum(BO_area),RU/sum(RU),RU_area/sum(RU_area))
  out<-apply(out,2,scales::percent,simplify = T)
  colnames(out)<-c('BO.N','BO.Area','RU.N','RU.Area')
  # out<-matrix(out,ncol = 4,byrow = F,dimnames = list(SOM=SOM_Label,method=c('BO.N','BO.A','RU.N','RU.A')))
  return(out)
})


for (ii in 1:3){
  postscript(file = paste0("C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/3x3_BO_map_split_",ii,".eps"),
             height = 50,
             width = 50,
             horizontal = F,
             bg = 'white',
  ...
  for (i in levels(SOM_Label)){
    ...
    if (length(TEMP)==0){
      ...
      mtext(paste0('N:',tableSOM[[ii]][i,1]), side=1,line = 0)
      mtext(paste0('Area:',tableSOM[[ii]][i,2]), side=1,line = 1)
      mtext(paste0('SOM: ',i), side=3,line=0) 
    }
   
  }
  dev.off()
  postscript(file = paste0("C:/Users/luigi/Documents/BH_TEST/MYELOMA/RUN_001/3x3_RU_map_split_",ii,".eps"),
             height = 50,
             width = 50,
             horizontal = F,
             bg = 'white',
  ...
  for (i in levels(SOM_Label)){
    ...
    if (length(TEMP)==0){
      ...
      mtext(paste0('N:',tableSOM[[ii]][i,3]), side=1,line = 0)
      mtext(paste0('Area:',tableSOM[[ii]][i,4]), side=1,line = 1)
      mtext(paste0('SOM: ',i), side=3,line=0) 
    }
...
mrkr<-colnames(geo_list_BO)[c(4,6,7,9,10,11,12,13)]
names(mrkr)<-mrkr

trsh<-lapply(mrkr,function(m){
  BO_TEMP<-lapply(uids,function(x){
    
    out<-(geo_list_BO[geo_list_BO$uid==x,m,drop=T])
    out<-trns$transform(out)
    out<-sort(out)
  })
  RU_TEMP<-lapply(uids,function(x){
    out<-(geo_list_RU[geo_list_RU$uid==x,m,drop=T])
    out<-trns$transform(out)
    out<-sort(out)
  })
  mean(c(unlist(lapply(BO_TEMP,quantile,0.5)),unlist(lapply(RU_TEMP,quantile,0.5))))
})

geo_list_BO_annot2<-geo_list_BO_annot
geo_list_RU_annot2<-geo_list_RU_annot
geo_list_BO_annot2$SOM<-'UNDF'
geo_list_RU_annot2$SOM<-'UNDF'


BO_TEMP<-flowCore::flowFrame(exprs = as.matrix(geo_list_BO_trans))
flowCore::write.FCS(BO_TEMP,
                    "D:/Luigi/FCS.Julie/BO.fcs")
RU_TEMP<-flowCore::flowFrame(exprs = as.matrix(geo_list_RU_trans))
flowCore::write.FCS(RU_TEMP,
                    "D:/Luigi/FCS.Julie/RU.fcs")




plot(NA,
     # xlim=c(0,max(vapply(BO_TEMP,length,c(1)),vapply(RU_TEMP,length,c(1)))),
     xlim=c(0,1),
     ylim=c(0,max(vapply(BO_TEMP,max,c(1)),vapply(RU_TEMP,max,c(1)))))
s<-1
lines(seq(0,1,length.out=length(BO_TEMP[[s]])),BO_TEMP[[s]])
lines(seq(0,1,length.out=length(RU_TEMP[[s]])),RU_TEMP[[s]])

mean(sf::st_area(geo_list_RU_annot[geo_list_RU_annot$uid==uids[1] & geo_list_RU_annot$SOM %in% lbl,]['GEOMETRY']),breaks = 100)
mean(sf::st_area(geo_list_BO_annot[geo_list_BO_annot$uid==uids[1] & geo_list_BO_annot$SOM %in% lbl,]['GEOMETRY']),breaks = 100)
