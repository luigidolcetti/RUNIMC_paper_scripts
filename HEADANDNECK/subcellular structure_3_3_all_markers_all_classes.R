targetFolder<-"C:/Users/luigi/Documents/BH_TEST/HEADANDNECK/RUN_001/SUBCELLULAR_3_allstars_table"
...
fsom<-readRDS(file = file.path("C:/Users/luigi/Documents/BH_TEST/HEADANDNECK/RUN_001/SUBCELLULAR_3",'som.R'  ))
...
TEMP<-(RUNIMC::quantNorm(rst[[uids[6]]]$x161dy.cd20.dy161di.,0.95))-
             (RUNIMC::quantNorm(rst[[uids[6]]]$x191ir.dna1.ir191di.,0.99))
...
raster::plot(TEMP,col=colorRampPalette(c('green','black','red'))(255),
             xlim=c(200,300),ylim=c(200,300),
             xaxt='n',
             yaxt='n',
             legend=F,
             bty='n')

plot(geo_list_BO_annot[geo_list_BO_annot$uid==uids[6] &
                       geo_list_BO_annot$SOM %in% c('22','21','16'),'GEOMETRY'],add=T,border='cyan',lwd=1)
plot(geo_list_RU_annot[geo_list_RU_annot$uid==uids[6] &
                         geo_list_RU_annot$SOM %in% c('22','21','16'),'GEOMETRY'],add=T,border='blue',lwd=1)
dev.off()
